...
const TYPEDESC_FILE_BASE = 'TypeDescriptions'; // uusi
...
function index_log(string $msg): void {
    ...
    @file_put_contents(index_app_log_path(), '['.$ts.'] '.$msg.PHP_EOL, FILE_APPEND | LOCK_EX);
}
...
function read_tag_text_rows(string $base): array {
    ...
    foreach ($lines as $i => $line) {
        ...
        $trim = ltrim($line);
        if ($trim === '' || str_starts_with($trim, '/*')) continue; // kommenttirivi
...
function write_full_log_row(array $row): void {
    ...
    index_log('CSV row written: finalVar='.(string)($row['finalVar'] ?? ''));
}

/* ========= Tila ========= */
$_SESSION['state'] = $_SESSION['state'] ?? 'intro';
$validStates = ['intro','quiz1','tiebreak','a1_result','phase2','phase2_tb','done2'];
if (!in_array($_SESSION['state'], $validStates, true)) $_SESSION['state'] = 'intro';

index_log('Request start: method='.($_SERVER['REQUEST_METHOD'] ?? '').' state='.($_SESSION['state'] ?? 'n/a').' ip='.($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
...
/* Testimoodi & admin-sivu */
if ($TEST_MODE && isset($_GET['admin'])) {
    $tb1  = load_tiebreak_questions(PHASE1_FILE_BASE);
    $p2   = load_phase2_blocks(PHASE2_FILE_BASE);
    $tb2a = load_phase2_tiebreak_all(PHASE2_FILE_BASE);

    $counts = ['A1'=>0,'B1'=>0,'C1'=>0,'D1'=>0];
    foreach ($phase1_all_preview as $q) $counts[$q['class']]++;

    ?>
    <!doctype html><html lang="fi"><head>
    <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Admin-tarkistus</title>
    <style>
      body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;margin:24px}
      table{border-collapse:collapse;margin:12px 0}
      th,td{border:1px solid #ccc;padding:8px 10px;text-align:left}
      .ok{color:#0a7c16}.warn{color:#b26b00}.err{color:#b00020}
      .small{color:#666}
    </style></head><body>
    <h1>Admin-tarkistus (testimoodi)</h1>
    <p>CSV (vaihe 1): <strong><?= $phase1_preview_path ? h($phase1_preview_path) : 'Ei löytynyt' ?></strong> — rivejä A1/B1/C1/D1: <strong><?= (int)$phase1_preview_count ?></strong></p>

    <h2>Vaihe 1 – luokkien määrät</h2>
    <table><tr><th>Luokka</th><th>Kpl</th><th>Status</th></tr>
    <?php foreach (['A1','B1','C1','D1'] as $k): ?>
      <tr><td><?=h($k)?></td><td><?= (int)$counts[$k] ?></td>
      <td class="<?= $counts[$k]>0?'ok':'err' ?>"><?= $counts[$k]>0?'OK':'PUUTTUU'?></td></tr>
    <?php endforeach; ?></table>

    <h2>Tiebreak – A11/B11/C11/D11</h2>
    <table><tr><th>Tagi</th><th>Kpl</th><th>Status</th></tr>
    <?php foreach (['A11','B11','C11','D11'] as $k): $n=count($tb1[$k]??[]);?>
      <tr><td><?=h($k)?></td><td><?=$n?></td>
      <td class="<?= $n>=2?'ok':($n>0?'warn':'err') ?>"><?= $n>=2?'OK (≥2)':($n>0?'Vähän':'PUUTTUU')?></td></tr>
    <?php endforeach; ?></table>

    <h2>Vaihe 2 – B2/C2/D2 caset</h2>
    <table><tr><th>Polku</th><th>Caseja</th><th>Status</th></tr>
    <?php foreach (['B2','C2','D2'] as $k): $n=count($p2[$k]??[]);?>
      <tr><td><?=h($k)?></td><td><?=$n?></td>
      <td class="<?= $n>0?'ok':'warn' ?>"><?= $n>0?'OK':'Ei caseta'?></td></tr>
    <?php endforeach; ?></table>

    <h2>Vaihe 2 – Tiebreak-caset (TB2/TC2/TD2)</h2>
    <table><tr><th>Ryhmä</th><th>Kpl</th><th>Status</th></tr>
    <?php foreach (['TB2','TC2','TD2'] as $g): $n=count($tb2a[$g]??[]);?>
      <tr><td><?=h($g)?></td><td><?=$n?></td>
      <td class="<?= $n>0?'ok':'warn' ?>"><?= $n>0?'OK':'Ei TB2/TC2/TD2-caseta'?></td></tr>
    <?php endforeach; ?></table>

    <h2>TypeDescriptions</h2>
    <p>Polku: <strong><?= $type_desc_path ? h($type_desc_path) : 'Ei löytynyt' ?></strong> — tyyppejä: <strong><?= (int)count($type_desc_map) ?></strong></p>
    <table><tr><th>Tyyppi</th><th>Kuvaus (ensimmäiset 80 merkkiä)</th></tr>
    <?php foreach (array_slice($type_desc_map, 0, 3, true) as $k=>$v): ?>
      <tr><td><?=h($k)?></td><td class="small"><?=h(mb_strimwidth($v,0,80,'…','UTF-8'))?></td></tr>
    <?php endforeach; ?></table>

    <p><a href="./?test=1<?= isset($_GET['showtestmode'])?'&showtestmode=1':'' ?>">Takaisin</a></p>
    </body></html>
    <?php
    exit;
}
...
/* ========= POST ========= */
// CSRF POISTETTU: Ei tarkistuksia
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_SESSION['state'] ?? '') === 'intro') && isset($_POST['start'])) {
    ...
    if (INVITES_ENABLED && $code !== '' && empty($_SESSION['InviteId'])) {
        ...
        if ($inv && (($inv['status'] ?? '') === 'sent' || ($inv['status'] ?? '') === 'accepted')) {
            ...
            $claim_ok = true;
            index_log('Invite claimed by code: id='.$_SESSION['InviteId']);
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_SESSION['state'] ?? '') === 'quiz1') && isset($_POST['choice'])) {
    ...
    if ($total === 0 || (!isset($_SESSION['nickname']) && !isset($_SESSION['ennea']))) {
        $_SESSION['state'] = 'intro';
        index_log('quiz1 -> intro (missing data)');
    }
    else {
        $n = (int)$_POST['choice'];
        if ($n>=1 && $n<=6) {
            ...
            if ($_SESSION['q1_index'] >= $total) {
                ...
                $max = reset($pts);
                $tops = array_keys(array_filter($pts, fn($v)=>$v===$max));
                ...
                if (count($tops) === 1) {
                    ...
                    if ($top === 'A1') {
                        ...
                        index_log('Phase1 winner A1 -> a1_result');
                    } else {
                        ...
                        index_log('Phase1 winner '.$top.' -> phase2 track='.$track);
                    }
                } else {
                    if (count($tops) > 2) {
                        $order = ['A1','B1','C1','D1'];
                        usort($tops, fn($a,$b)=>array_search($a,$order,true) <=> array_search($b,$order,true));
                    ...
                    index_log('Phase1 tie -> tiebreak (classes='.implode(',',$tops).')');
                }
            }
        }
    }
        ...
        if ($blk) {
            ...
            if ($_SESSION['tiebreak_index'] >= count($set)) {
                ...
                if (count($tops) === 1) $top = $tops[0];
                    ...
                    $eq = array_filter($tops, fn($c)=>($yc[$c]??0)===($yc[$best]??0));
                ...
                if ($top === 'A1') {
                    ...
                    write_full_log_row($_SESSION['log_row']);
                    $_SESSION['state'] = 'a1_result';
                    index_log('Tiebreak winner A1 -> a1_result');
                } else {
                    $_SESSION['cases_all'] = load_phase2_blocks(PHASE2_FILE_BASE);
                    $track = $top[0] . '2';
                    $_SESSION['track']     = $track;
                    ...
                    $_SESSION['state'] = 'phase2';
                    index_log('Tiebreak winner '.$top.' -> phase2 track='.$track);
                }
            }
        }
    }
        ...
        if ($blk) {
            ...
            if ($_SESSION['q2_index'] >= count($_SESSION['blocks2'])) {
                ...
                if (!empty($vp)) {
                    ...
                    $leaders = array_keys(array_filter($vp, fn($v)=>$v===$max));
                } else $leaders = [];
                ...
                if (count($leaders) <= 1) {
                    ...
                    index_log('Phase2 done finalVar='.$final.' -> done2');
                } else {
                    natsort($leaders);
                    ...
                    if (!empty($filtered)) {
                        ...
                        index_log('Phase2 tie -> phase2_tb (pair='.implode(' vs ', $pair).')');
                    } else {
                        ...
                        write_full_log_row($_SESSION['log_row']);
                        $_SESSION['state'] = 'done2';
                        index_log('Phase2 tie no TB2 cases -> random pick='.$pick.' -> done2');
                    }
                }
            }
        } else {
            ...
            write_full_log_row($_SESSION['log_row']);
            $_SESSION['state'] = 'done2';
            index_log('Phase2 missing block -> done2');
        }
    }
}
    ...
    if ($raw >= -4 && $raw <= 4) {
        ...
        if ($blk) {
            ...
            if ($_SESSION['tb2_index'] >= count($_SESSION['tb2_set'])) {
                ...
                write_full_log_row($_SESSION['log_row']);
                $_SESSION['state'] = 'done2';
                index_log('Phase2 TB finished finalVar='.$final.' -> done2');
            }
        }
    }
}
...
if (INVITES_ENABLED && ($_SESSION['state'] ?? '') === 'intro' && isset($_GET['invite'])) {
    ...
    if ($token !== '') {
        ...
        if ($inv && (($inv['status'] ?? '') === 'sent' || ($inv['status'] ?? '') === 'accepted')) {
            ...
            index_log('Invite token invalid');
        }
    }
}
...
.footer{margin-top:16px;display:flex;gap:12px;flex-wrap:wrap;justify-content:flex-end}
...
<?php elseif (($_SESSION['state'] ?? '') === 'a1_result'): ?>
  ...
  <?php $desc = $type_desc_map['4'] ?? ''; if ($desc!==''): ?>
...
<?php elseif (($_SESSION['state'] ?? '') === 'done2'):
    ...
    $nonzero = array_filter($vars, fn($v)=>$v>0);
    $topVar  = $_SESSION['log_row']['finalVar'] ?? ( $nonzero ? array_key_first($nonzero) : '' );
