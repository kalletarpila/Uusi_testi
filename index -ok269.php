...
function check_tiebreak_needed(): bool {
    ...
    // Jos vain yksi korkein, ei tiebreakiä tarvita
    return count($tops) > 1;
}
...
function recalculate_tiebreak_points(): void {
    ...
    // Restore original Phase 1 points (before tiebreak)
    if (isset($_SESSION['phase1_points_backup'])) {
        $_SESSION['points1'] = $_SESSION['phase1_points_backup'];
        $_SESSION['yesCounts1'] = $_SESSION['phase1_yes_backup'] ?? [];
    }
    ...
    foreach ($answers as $index => $choice) {
        $blk = $set[$index] ?? null;
        if ($blk && $choice >= 1 && $choice <= 6) {
            $cls = $blk['class'];
            $question_number = $blk['question_number'] ?? 'unknown';
            
            $_SESSION['points1'][$cls] = ($_SESSION['points1'][$cls] ?? 0) + $choice;
            if ($choice >= YES_MIN) {
                $_SESSION['yesCounts1'][$cls] = ($_SESSION['yesCounts1'][$cls] ?? 0) + 1;
            }
...
} elseif (($_SESSION['state'] ?? '') === 'tiebreak' && isset($_POST['return_to_phase1'])) {
  index_log('POST: return_to_phase1 received, value=' . ($_POST['return_to_phase1'] ?? 'NULL'));
  handle_return_to_phase1();
}
        ...
        <?php endif; ?>
          ...
          if (
            ...
            if (count(array_diff($tops, $last_classes)) === 0 && count($tops) === count($last_classes)) {
              ?>
              <form method="post" style="margin: 0;">
                <input type="hidden" name="return_to_tiebreak" value="1">
                <button type="submit" class="btn" style="font-size: 14px; padding: 8px 12px; background: #e7f3ff; color: #0066cc; border: 1px solid #b3d7ff;">
                  Seuraava &rarr;
                </button>
              </form>
              <?php
  ...
  <?php else:
      ...
      <div class="question-layout">
        ...
        <div class="navigation-controls" style="display: flex; justify-content: space-between; align-items: center; margin: 20px 0; gap: 10px;">
          ...
          <?php else: ?>
            <form method="post" style="margin: 0;">
              <input type="hidden" name="return_to_phase1" value="<?= min($_SESSION['q1_index'] ?? 0, (count($_SESSION['phase1']) ?? 1)-1) ?>">
              <button type="submit" class="btn" style="font-size: 14px; padding: 8px 12px;">
                ← Edellinen
              </button>
            </form>
...
<?php elseif (($_SESSION['state'] ?? '') === 'phase2_intro'): ?>
  <div class="progress-wrap">
    <div class="progress-label">Vaihe 2/2</div>
    <div class="progress">
      <div class="progress-bar" style="width: 0%;"></div>
    </div>
  </div>

  <h2>Vaihe 2: Syventävät kysymykset</h2>
  
  <div style="background: #f8f9fa; border: 1px solid #e9ecef; border-radius: 8px; padding: 20px; margin: 20px 0; line-height: 1.6;">
    <h3 style="margin-top: 0; color: #495057;">Miten vaihe 2 toimii?</h3>
    
    <p><strong>Kysymystyypit:</strong> Seuraavaksi saat nähdä useita tilannekuvauksia, ja kaksi kaksi erilaista käyttäytymistapaa tai ajattelutapaa tällaisessa tilannteessa.</p>
...
<?php endif; ?>
  ...
  <div style="margin-top: 10px; text-align: center; font-size: 12px; color: #666;">
    <a href="tietosuojalauseke.pdf" target="_blank" style="color: #666; text-decoration: underline;">Tietosuojalauseke</a>
  </div>
  <?php endif; ?>
</div>
